...
void getBranchMidcode(string rs, string rt, string label, int op, bool meet) {
	if (meet) {
		// 如果满足条件，则跳转至label
		switch (op) {
		case LSS:
			midCodeTable.push_back(midCode(midCodeOp::BLT, label, rs, rt));
			break;
		case LEQ:
			midCodeTable.push_back(midCode(midCodeOp::BLE, label, rs, rt));
			break;
		case GRE:
			midCodeTable.push_back(midCode(midCodeOp::BLT, label, rt, rs));
			break;
		case GEQ:
			midCodeTable.push_back(midCode(midCodeOp::BLE, label, rt, rs));
			break;
		case EQL:
			midCodeTable.push_back(midCode(midCodeOp::BEQ, label, rs, rt));
			break;
		case NEQ:
			midCodeTable.push_back(midCode(midCodeOp::BNE, label, rs, rt));
			break;
		default:
			break;
		}
...
void Parser::varDefWithInitial() {
	int col_count = 0, row_count = 0, index = 0;
	...
	if (curToken->isType(ASSIGN)) {
		...
		if (curToken->isType(ASSIGN)) {
			...
			if ((ste.valueType == ValueType::CHAR && !curToken->isType(CHARCON))
				...
				type_unmatch = true;
			constant(&value);
			midCodeTable.push_back(midCode(midCodeOp::ARRSAVE, ste.name, int2string(col_count), int2string(value)));
			node.addNode(subNode);
			col_count++;
			while (curToken->isType(COMMA)) {
				checkToken(COMMA, "<COMMA>", &node);
			...
			if ((ste.valueType == ValueType::CHAR && !curToken->isType(CHARCON))
				...
				type_unmatch = true;
			constant(&value);
			index = row_count * ste.arrayInfo.array_col + col_count;
			midCodeTable.push_back(midCode(midCodeOp::ARRSAVE, ste.name, int2string(index), int2string(value)));
			node.addNode(subNode);
			col_count++;
...
void Parser::funcWithReturn() {
	...
	midCodeTable.push_back(midCode(midCodeOp::FUNC, ste.name));
	...
	midCodeTable.push_back(midCode(midCodeOp::ENDFUNC));
...
void Parser::paramList(STE* ste) {
	...
	if (curToken->isType(INTTK) || curToken->isType(CHARTK)) {
		...
		midCodeTable.push_back(midCode(midCodeOp::MIDPARAM, param_ste.name, "", ""));
...
bool Parser::factor(string* ansTemp) {
	...
	if (TOKEN_PEEK(0).isType(IDENFR) && TOKEN_PEEK(1).isType(LPARENT)) {
		...
		*ansTemp = getTempVar();
		symbolTableManager.insert(STE(*ansTemp, localAddr, VAR, INTEGER, 0, 1));
		localAddr++;
		midCodeTable.push_back(midCode(midCodeOp::MOVE, *ansTemp, "$v0"));
	}
		...
		if (ste != nullptr && dim == 1) {
			*ansTemp = getTempVar();
			symbolTableManager.insert(STE(*ansTemp, localAddr, VAR, INTEGER, 0, 1));
			localAddr++;
			midCodeTable.push_back(midCode(midCodeOp::ARRLOAD, *ansTemp, ste->name, i));
		}
		else if (ste != nullptr && dim == 2) {
			string temp = getTempVar();
			symbolTableManager.insert(STE(temp, localAddr, VAR, INTEGER, 0, 1));
			localAddr++;
			midCodeTable.push_back(midCode(midCodeOp::MULTOP, temp, i, int2string(ste->arrayInfo.array_col)));

			index = getTempVar();
			symbolTableManager.insert(STE(index, localAddr, VAR, INTEGER, 0, 1));
			localAddr++;
			midCodeTable.push_back(midCode(midCodeOp::ADD, index, j, temp));

			*ansTemp = getTempVar();
			symbolTableManager.insert(STE(*ansTemp, localAddr, VAR, INTEGER, 0, 1));
			localAddr++;
			midCodeTable.push_back(midCode(midCodeOp::ARRLOAD, *ansTemp, ste->name, index));
		}
	}
...
void Parser::assignStatement() {
	...
	if (dim == 0) {
	...
	else if (ste != nullptr && dim == 1) {
		midCodeTable.push_back(midCode(midCodeOp::ARRSAVE, z, i, x));
	}
	else if (ste != nullptr && dim == 2) {
		string temp = getTempVar();
		symbolTableManager.insert(STE(temp, localAddr, VAR, INTEGER, 0, 1));
		...
		midCodeTable.push_back(midCode(midCodeOp::ARRSAVE, z, index, x));
	}
...
void Parser::ifStatement() {
	...
	getBranchMidcode(rs, rt, label1, op, false);
	...
	if (curToken->isType(ELSETK)) {
		...
		midCodeTable.push_back(midCode(midCodeOp::GOTO, label2));
		midCodeTable.push_back(midCode(midCodeOp::LABEL, label1));
		checkToken(ELSETK, "<ELSETK>", &node);
		statement();
		node.addNode(subNode);
		midCodeTable.push_back(midCode(midCodeOp::LABEL, label2));
	}
	else {
		midCodeTable.push_back(midCode(midCodeOp::LABEL, label1));
	}
...
*/
void Parser::condition(string* rs, string* rt, int* op) {
	AST_node node("<条件>", true);
	bool isChar = false;
	bool ischar;

	ischar = expression(rs);
	node.addNode(subNode);
...
void Parser::loopStatement() {
	...
	string rs, rt;
	int op;
	int index_con, index_endcon;
	string beginLabel = getLabel();
	...
	if (curToken->isType(WHILETK)) {
		...
		getBranchMidcode(rs, rt, endLabel, op, false);
		midCodeTable.push_back(midCode(midCodeOp::LABEL, beginLabel));
		...
		index_endcon = tokenptr;
		tokenptr = index_con;
		curToken = &tokenList[tokenptr];
		condition(&rs, &rt, &op);
		tokenptr = index_endcon;
		curToken = &tokenList[tokenptr];
		getBranchMidcode(rs, rt, beginLabel, op, true);
		midCodeTable.push_back(midCode(midCodeOp::LABEL, endLabel));
	}
	else {
		string i;
		int stepVal, stepOp;
		...
		midCodeTable.push_back(midCode(midCodeOp::MOVE, i, temp));
		...
		if (stepOp == PLUS) 
			midCodeTable.push_back(midCode(midCodeOp::ADD, i, i, int2string(stepVal)));
		else
			midCodeTable.push_back(midCode(midCodeOp::SUB, i, i, int2string(stepVal)));
...
FIRST CASETK
*/
void Parser::caseList(bool ischar, string endlabel, string exp) {
	AST_node node("<情况表>", true);
	caseStatement(ischar, endlabel, exp);
	node.addNode(subNode);
...
FIRST CASETK
*/
void Parser::caseStatement(bool ischar, string endlabel, string exp) {
	int value;
	string endcase = getLabel();
	...
	midCodeTable.push_back(midCode(midCodeOp::BNE, endcase, exp, int2string(value)));
	...
	midCodeTable.push_back(midCode(midCodeOp::GOTO, endlabel));
	midCodeTable.push_back(midCode(midCodeOp::LABEL, endcase));
...
void Parser::callFuncWithReturn() {
	...
	midCodeTable.push_back(midCode(midCodeOp::CALL, ste->name));
...
void Parser::valueParamList(string funcName) {
	...
	if (curToken->isType(IDENFR) || curToken->isType(LPARENT) || curToken->isType(PLUS)
		...
		midCodeTable.push_back(midCode(midCodeOp::PUSHPARAM, temp));
		node.addNode(subNode);
		if (iter != args.end()) {
...
void Parser::returnStatement() {
	...
	if (curToken->isType(LPARENT)) {
		...
		if (curToken->isType(RPARENT) && symbolTableManager.getPosType() != ValueType::VOID) {
			...
			midCodeTable.push_back(midCode(midCodeOp::MOVE, "$v0", temp));
			midCodeTable.push_back(midCode(midCodeOp::RET));
