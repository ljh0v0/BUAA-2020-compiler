...
struct STE
{
	...
	int value;				// 常量的值
	int length;				//数组：数组长度；函数：函数有多少变量（参数+局部变量+临时)；
	int addr;				// 分配的内存地址
	bool isGlobal;			// 全局变量
...
public:
	STE(string n = "", int a = -1, IdentType iType = IdentType::VAR, ValueType vType = ValueType::INTEGER, int v = 0, int l = 0, bool isglobal = false) :
		name(n), identType(iType), valueType(vType), value(v), length(l), addr(a), isGlobal(isglobal){
		father = NULL;
		arrayInfo.array_dim = 0;
		arrayInfo.array_col = 0;
		arrayInfo.array_row = 0;
		args = vector<Argument>();
		children = map<string, STE>();
	};
};
